...
// VOICE AI SYSTEM
// =========================
const activeListeners = new Map(); // guildId -> Set of userIds being listened to
...
async function generateSpeech(text) {
  try {
    const response = await fetch('https://api.openai.com/v1/audio/speech', {
      ...
      body: JSON.stringify({
        model: 'tts-1',
        voice: 'onyx', // deep/robotic — fits JARVIS vibe
        input: text,
        speed: 1.0,
      }),
    });
...
function listenToUser(connection, userId, guildId, member) {
  ...
  receiver.speaking.on('start', (speakingUserId) => {
    ...
    audioStream.on('end', async () => {
      ...
      const activeMode = getActiveMode(guildId2);
...
You are JARVIS, a voice assistant in a Discord voice channel. 
Keep replies SHORT — 1-3 sentences max. You are speaking out loud so no markdown, no bullet points, no emojis.
Be natural and conversational. Never say @everyone or @here.`
            },
        ...
        aiResponse = res.choices[0].message.content
          .replace(/@everyone/gi, '')
          .replace(/@here/gi, '')
          .replace(/[*_`#]/g, ''); // strip markdown for TTS
      } catch (err) {
        console.error('[Voice] AI response failed:', err.message);
        return;
      }

      if (!aiResponse) return;
      console.log(`[Voice] JARVIS response: ${aiResponse}`);
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (!voiceChannel) {
    return interaction.reply({ content: '❌ You need to be in a voice channel first.', flags: 64 });
  }

  const existing = getVoiceConnection(interaction.guild.id);
  if (existing) {
    return interaction.reply({ content: '⚠️ Already in a voice channel. Use `/leave` first.', flags: 64 });
  }
  ...
  // Listen to everyone currently in the channel + anyone who joins
  ...
  // Also pick up new users who join later
  client.on('voiceStateUpdate', (oldState, newState) => {
    if (newState.guild.id !== interaction.guild.id) return;
    if (newState.channelId !== voiceChannel.id) return;
    if (newState.member?.user.bot) return;
    listenToUser(connection, newState.member.id, interaction.guild.id, newState.member);
  });
  ...
  return interaction.reply(`🎙️ Joined **${voiceChannel.name}**! I'm listening — talk to me!`);
}
